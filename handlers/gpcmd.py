...
###############
  if text:
    if text == c.delrpmsg and message.reply_to_message:
      ...
      Bot("deleteMessage",{"chat_id":chatID,"message_id":message.reply_to_message.id})

    if text == c.settingsCmd and Ckuser(message):
      kb = st(client, message,redis)
      Bot("sendMessage",{"chat_id":chatID,"text":r.settings.format(title),"reply_to_message_id":message.id,"parse_mode":"html","reply_markup":kb})


    if re.search(c.settingsCmdRes, text) and Ckuser(message):
      kb = st_res(client, message,redis)
      Bot("sendMessage",{"chat_id":chatID,"text":r.settingsRes.format(title),"reply_to_message_id":message.id,"parse_mode":"html","reply_markup":kb})
    ...
    if rank != "admin":
      ...
      if re.search(c.setname, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Dsetname.format(name),"reply_to_message_id":message.id,"parse_mode":"html"})

      if re.search(c.setabout, text):
        about = text.replace(c.Dsetabout,"")
        Bot("setChatDescription",{"chat_id":chatID,"description":about})
        Bot("sendMessage",{"chat_id":chatID,"text":r.Dsetabout.format(about),"reply_to_message_id":message.id,"parse_mode":"html"})
      ...
      if re.search(c.setphoto, text) and message.reply_to_message and message.reply_to_message.photo:
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Dsetphoto,"reply_to_message_id":message.id,"parse_mode":"html"})

      if re.search("^حذف الصورة$|^حذف الصوره$", text):
        client.delete_chat_photo(chat_id=chatID)
        Bot("sendMessage",{"chat_id":chatID,"text":"☑️꒐ تم حذف صورة المجموعة","reply_to_message_id":message.id,"parse_mode":"html"})
      ...
      if re.search(c.remallR, text):
        ...
        try:
          ...
          BY = "<a href=\"tg://user?id={}\">{}</a>".format(userId,userFn)
          Bot("sendMessage",{"chat_id":chatID,"text":r.remall.format(BY,tx),"reply_to_message_id":message.id,"parse_mode":"html"})
        except Exception as e:
          Bot("sendMessage",{"chat_id":chatID,"text":r.userNocc,"reply_to_message_id":message.id,"parse_mode":"html"})
      if re.search(c.floodset, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Tfset,"reply_to_message_id":message.id,"parse_mode":"html","reply_markup":kb})
      ...
      if re.search(c.twostepset, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Ttset,"reply_to_message_id":message.id,"parse_mode":"html","reply_markup":kb})
...
<a href=\"https://t.me/XXCBB/\">Mohammed</a>
[Mohammed](https://t.me/XXCBB/)
...
⎯ ⎯ ⎯ ⎯""",parse_mode="markdown",disable_web_page_preview=True)
      if re.search(c.setIDC, text):
          ...
          age = getAge(userID,r)
          v = Bot("sendMessage",{"chat_id":chatID,"text":tx.format(username=("@"+username or "None"),id=userID,stast=t,msgs=msgs,edits=edits,age=age,rate=str(rate)+"%"),"reply_to_message_id":message.id,"parse_mode":"html"})
          if v["ok"]:
            redis.hset("{}Nbot:SHOWid".format(BOT_ID),chatID,tx)
            Bot("sendMessage",{"chat_id":chatID,"text":r.DsetIDShow,"reply_to_message_id":message.id,"parse_mode":"html"})
          elif v["ok"] == False:
      ...
      if re.search(c.block, text):
        if re.search(c.block2, text):
          ...
          if redis.sismember("{}Nbot:{}:blockTEXTs".format(BOT_ID,chatID),tx):
            Bot("sendMessage",{"chat_id":chatID,"text":r.Adoneblock.format(tx),"reply_to_message_id":message.id,"parse_mode":"html"})
          else:
            redis.sadd("{}Nbot:{}:blockTEXTs".format(BOT_ID,chatID),tx)
            Bot("sendMessage",{"chat_id":chatID,"text":r.Doneblock.format(tx,title),"reply_to_message_id":message.id,"parse_mode":"html"})

        if message.reply_to_message:
          if message.reply_to_message.sticker:
            ID = message.reply_to_message.sticker.file_id
            if redis.sismember("{}Nbot:{}:blockSTICKERs".format(BOT_ID,chatID),ID):
              Bot("sendMessage",{"chat_id":chatID,"text":r.StA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
            else:
              redis.sadd("{}Nbot:{}:blockSTICKERs".format(BOT_ID,chatID),ID)
              Bot("sendMessage",{"chat_id":chatID,"text":r.StB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
          
          if message.reply_to_message.photo:
            ID = message.reply_to_message.photo.file_id
            if redis.sismember("{}Nbot:{}:blockphotos".format(BOT_ID,chatID),ID):
              Bot("sendMessage",{"chat_id":chatID,"text":r.PhA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
            else:
              redis.sadd("{}Nbot:{}:blockphotos".format(BOT_ID,chatID),ID)
              Bot("sendMessage",{"chat_id":chatID,"text":r.PhB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
          
          if message.reply_to_message.animation:
            ID = message.reply_to_message.animation.file_id
            if redis.sismember("{}Nbot:{}:blockanimations".format(BOT_ID,chatID),ID):
              Bot("sendMessage",{"chat_id":chatID,"text":r.GfA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
            else:
              redis.sadd("{}Nbot:{}:blockanimations".format(BOT_ID,chatID),ID)
              Bot("sendMessage",{"chat_id":chatID,"text":r.GfB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
      ...
      if re.search(c.unblock, text):
        if re.search(c.unblock2, text):
          ...
          if redis.sismember("{}Nbot:{}:blockTEXTs".format(BOT_ID,chatID),tx):
            ...
            Bot("sendMessage",{"chat_id":chatID,"text":r.unDoneblock.format(tx,title),"reply_to_message_id":message.id,"parse_mode":"html"})
          else:
            Bot("sendMessage",{"chat_id":chatID,"text":r.unAdoneblock.format(tx),"reply_to_message_id":message.id,"parse_mode":"html"})
        ...
        if message.reply_to_message:
          if message.reply_to_message.sticker:
            ...
            if redis.sismember("{}Nbot:{}:blockSTICKERs".format(BOT_ID,chatID),ID):
              ...
              Bot("sendMessage",{"chat_id":chatID,"text":r.unStB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
            else:
              Bot("sendMessage",{"chat_id":chatID,"text":r.unStA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})

          if message.reply_to_message.photo:
            ID = message.reply_to_message.photo.file_id
            if redis.sismember("{}Nbot:{}:blockphotos".format(BOT_ID,chatID),ID):
              redis.srem("{}Nbot:{}:blockphotos".format(BOT_ID,chatID),ID)
              Bot("sendMessage",{"chat_id":chatID,"text":r.unPhB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})       
            else:
              Bot("sendMessage",{"chat_id":chatID,"text":r.unPhA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
          
          if message.reply_to_message.animation:
            ID = message.reply_to_message.animation.file_id
            if redis.sismember("{}Nbot:{}:blockanimations".format(BOT_ID,chatID),ID):
              redis.srem("{}Nbot:{}:blockanimations".format(BOT_ID,chatID),ID)
              Bot("sendMessage",{"chat_id":chatID,"text":r.unGfB.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
            else:
              Bot("sendMessage",{"chat_id":chatID,"text":r.unGfA.format(title),"reply_to_message_id":message.reply_to_message.id,"parse_mode":"html"})
      ...
      if re.search(c.Blocklist, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.blocklist.format(r.blocklist2,title),"reply_to_message_id":message.id,"reply_markup":reply_markup})
      ...
      if re.search(c.Replylist, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.blocklist.format(text,title),"reply_to_message_id":message.id,"reply_markup":reply_markup})

      if re.search(c.FloodT, text):
        Nu = text.split(" ")[2]
        redis.hset("{}Nbot:time_ck".format(BOT_ID),chatID,Nu)
        Bot("sendMessage",{"chat_id":chatID,"text":r.DoneSet.format(text.split(" ")[0]+" "+text.split(" ")[1],Nu),"reply_to_message_id":message.id,"parse_mode":"html"})
      ...
      if re.search(c.STWEL, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Donewel.format(Wc),"reply_to_message_id":message.id,"parse_mode":"html"})
      ...
      if re.search(c.STreply, text):
        ...
        if redis.hexists("{}Nbot:{}:TXreplys".format(BOT_ID,chatID),tx):
          ...
          kb = InlineKeyboardMarkup([[InlineKeyboardButton(r.MoreInfo, url="t.me/alsh_3k")]])
    ...
    if re.search(c.pinmsg, text) and message.reply_to_message:
      if not redis.sismember("{}Nbot:Lpin".format(BOT_ID),chatID):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.Dpinmsg,"reply_to_message_id":message.id,"parse_mode":"html"})
      ...
      if re.search(c.unpinmsg, text):
        if not redis.sismember("{}Nbot:Lpin".format(BOT_ID),chatID):
          ...
          Bot("sendMessage",{"chat_id":chatID,"text":r.Dunpinmsg,"reply_to_message_id":message.id,"parse_mode":"html"})
    ...
    if re.search(c.SETlink, text):
      ...
      Bot("sendMessage",{"chat_id":chatID,"text":r.Dsetlk.format(lk),"reply_to_message_id":message.id,"parse_mode":"html","disable_web_page_preview":True})

#cre
    if rank != "admin" and rank != "owner":
      if re.search(c.deletebots, text):
        bots = [x for x in client.get_chat_members(chatID,filter=enums.ChatMembersFilter.BOTS) if x.user.is_bot and x.user.id !=int(BOT_ID) and x.status != "administrator"]
        if bots:
          Bot("sendMessage",{"chat_id":chatID,"text":r.LenBots.format(len(bots)),"reply_to_message_id":message.id,"parse_mode":"html"})
          for u in bots:
            Bot("kickChatMember",{"chat_id":chatID,"user_id":u.user.id,"until_date":int(time.time() + 60)})
            time.sleep(0.3)
        else:
          Bot("sendMessage",{"chat_id":chatID,"text":r.NoBots,"reply_to_message_id":message.id,"parse_mode":"html"})
      
      if re.search(c.deletebans, text):
        kicked = [x for x in client.get_chat_members(chatID,filter=enums.ChatMembersFilter.BANNED)]
        if kicked:
          Bot("sendMessage",{"chat_id":chatID,"text":r.Lenbans.format(len(kicked)),"reply_to_message_id":message.id,"parse_mode":"html"})
          for u in kicked:
            Bot("unbanChatMember",{"chat_id":chatID,"user_id":u.user.id})
            redis.srem("{}Nbot:{}:bans".format(BOT_ID,chatID),u.user.id)
            time.sleep(0.3)
        else:
          Bot("sendMessage",{"chat_id":chatID,"text":r.NobansC,"reply_to_message_id":message.id,"parse_mode":"html"})
      

      if re.search(c.deleterks, text):
        restricted = [x for x in client.get_chat_members(chatID,filter=enums.ChatMembersFilter.RESTRICTED)]
        if restricted:
          Bot("sendMessage",{"chat_id":chatID,"text":r.Lenrks.format(len(restricted)),"reply_to_message_id":message.id,"parse_mode":"html"})
          for u in restricted:
            Bot("restrictChatMember",{"chat_id": chatID,"user_id": userId,"can_send_messages": 1,"can_send_media_messages": 1,"can_send_other_messages": 1,"can_send_polls": 1,
          "can_change_info": 1,"can_add_web_page_previews": 1,"can_pin_messages": 1,"can_invite_users": 1,})
          ...
          Bot("sendMessage",{"chat_id":chatID,"text":r.NorksC,"reply_to_message_id":message.id,"parse_mode":"html"})
        
      if re.search(c.deleteDeleted, text):
        deleted = [x for x in client.get_chat_members(chatID) if x.user.is_deleted]
        if deleted:
          Bot("sendMessage",{"chat_id":chatID,"text":r.LenDeleted.format(len(deleted)),"reply_to_message_id":message.id,"parse_mode":"html"})
          for u in deleted:
            Bot("kickChatMember",{"chat_id":chatID,"user_id":u.user.id,"until_date":int(time.time() + 60)})
            time.sleep(0.3)
        else:
          Bot("sendMessage",{"chat_id":chatID,"text":r.NoDeleted,"reply_to_message_id":message.id,"parse_mode":"html"})
      
      if re.search(c.delmsgs, text):
              lim = text.split(" ")[1]
              ids = []
              if message.reply_to_message:
                nu = message.reply_to_message.id
      ...
      if re.search(c.PROadmins, text):
        ...
        Bot("sendMessage",{"chat_id":chatID,"text":r.DPROadmins,"reply_to_message_id":message.id,"parse_mode":"html"})
